...
local function createCard(cardsArray)

end

function Draw.new(hud, playerScript)
    local texture = ResourceManager:getResource('card_back')
    local self = setmetatable(Draw, Card)
    local button = hud:addButton(Vector2f.new(400, 90), Vector2f.new(0.8, 0.8), texture)
...
function Draw:initState()
    self.button:setCallback(function()
        self.playerScript:call("createCard")
    end)
end
