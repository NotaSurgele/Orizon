...
public:
    ...
    HashGrid(float const& cellSize=100, int capacity = 100) : _size(0),
                        _cellSize(cellSize), _capacity(capacity) {
    ...
    void insert(Entity* entity) {
        std::vector<sf::Vector2f> cells = calculateCells(entity);
    ...
    std::vector<Entity *> retrieve(Entity *e) {
        std::vector<sf::Vector2f> cells = calculateCells(e);
...
private:
    ...
    std::vector<sf::Vector2f> calculateCells(Entity *e) {
    ...
    void resize() {
        _capacity *= 2;
        std::unordered_map<sf::Vector2f, std::vector<Entity *>, VectorHash> newGrid;
...
private:
    ...
    std::unordered_map<sf::Vector2f, std::vector<Entity *>, VectorHash> _grid;
};
